...
export function needsMigration(db) {
  ...
  if (!coll.tiles) return false;
  const hasUnmigratedTiles = coll.tiles.some(t => !t.recipeVersionId && t.recipe);
  return hasUnmigratedTiles || (coll.recipes.length === 0 && coll.tiles.length > 0);
}
...
export function runMigration(db) {
  ensureRecipeCollections(db);
  const coll = getCollections(db);
  if (!coll.tiles || coll.tiles.length === 0) {
