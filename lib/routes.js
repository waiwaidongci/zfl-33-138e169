...
import { validateStockForDeduction, deductStock, ensureInventoryCollection, findStockByNameAndBatchNo } from "./inventory-repository.js";
...
import { handleUpdateTileWithStatus, handleTransitionStatus } from "./tile-status-routes.js";
...
export async function validateTileBusinessRules(db, input, { autoCreateRecipe = true } = {}) {
  ...
  if (materialBatchRefs && batchWeight) {
    ...
    if (!validation.valid) {
      ...
      result._stockDeductions = validation.deductions;
    }
...
export async function handleImportCommit(input, db) {
  ...
  for (const tile of inserted) {
    ...
    if (stockDeductions && !tile.inventoryDeducted) {
      deductStock(db, stockDeductions);
...
async function handleApplyPlanAsBatch(id, input, db, plan) {
  ...
  for (const result of tileResults) {
    if (result.success) {
      const transResult = await handleTransitionStatus(
        result.tile.id,
        {
      ...
      if (transResult.status === 200) {
        transitionResults.push({ id: result.tile.id, ...transResult.data });
      } else {
        transitionErrors.push({ id: result.tile.id, ...transResult.data });
      }
    }
  }
  ...
  if (transitionErrors.length > 0) {
    return {
      ...
      data: {
        ...
        message: `${transitionErrors.length} 个试片状态推进失败`,
        batchId: batch.id,
        transitionResults,
        transitionErrors
      }
...
async function processBatchTile(tileInput, plan, db, stockDeductionMap, index) {
  ...
  if (tileInput.tileId) {
    ...
    if (tileInput.materialBatchRefs && tileInput.batchWeight && !existingDeductions) {
      ...
      existingTile.materialBatchRefs = validation.deductions.map(d => ({
        ingredientName: d.ingredientName,
        batchNo: d.batchNo,
        deducted: d.requiredQuantity,
        unit: d.unit
      }));
      existingTile.batchWeight = Number(tileInput.batchWeight);
      existingTile.firingCurve = plan.firingCurve;
      existingTile.peakTemp = plan.peakTemp;
      existingTile.kiln = plan.kiln || existingTile.kiln;
      existingTile.fromPlanId = plan.id;
      return {
        success: true,
        tile: existingTile,
    ...
    existingTile.kiln = plan.kiln || existingTile.kiln;
    existingTile.fromPlanId = plan.id;
    return { success: true, tile: existingTile };
  }
