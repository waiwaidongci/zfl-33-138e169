...
import { generateExperimentReview } from "./experiment-review.js";
...
export function routesInfo() {
  return {
    ...
    endpoints: [
      ...
      "GET /events/types",
      "GET /events/stats",
      "GET /events/type/:type?limit=&offset=",
      "GET /events/timeline/:entityId?type=&entityType=&from=&to=&limit=&offset=&order=",
      "GET /tiles/:id/review?topSimilar=&minSimilarity=",
      "POST /tiles/review"
    ]
  };
}
...
export async function handleCreateTile(input, db) {
  ...
  recordEvent(db, {
    ...
    entityId: tile.id,
    entityType: ENTITY_TYPES.TILE,
    payload: {
      body: tile.body,
      recipe: tile.recipe,
      recipeVersionId: tile.recipeVersionId,
...
async function handleApplyPlanAsSingleTile(id, input, db, plan) {
  ...
  recordEvent(db, {
    ...
    operator: "system",
    note: `基于烧成规划 ${plan.id} 创建试片 ${tile.id}`
  });
...
export async function handleGetExperimentReview(tileId, url, db) {
  if (!tileId || typeof tileId !== "string" || tileId.trim() === "") {
    return {
      status: 400,
      data: { error: "missing_tile_id", message: "试片ID为必填" }
    };
  }
  const options = {};
  const topSimilar = url.searchParams.get("topSimilar");
  const minSimilarity = url.searchParams.get("minSimilarity");
  if (topSimilar !== null && !isNaN(Number(topSimilar))) {
    options.topSimilar = Math.max(1, Math.min(20, Number(topSimilar)));
  }
  if (minSimilarity !== null && !isNaN(Number(minSimilarity))) {
    options.minSimilarity = Math.max(0, Math.min(100, Number(minSimilarity)));
  }
  const result = generateExperimentReview(db, tileId, options);
  if (result.error) {
    if (result.error === "tile_not_found") {
...
export async function handlePostExperimentReview(input, db) {
  if (!input || typeof input !== "object") {
    return {
      status: 400,
      data: { error: "invalid_input", message: "请求体必须为JSON对象" }
    };
  }
  const tile = input.tile;
  const tileId = input.tileId;
  const options = {};
  if (input.topSimilar !== undefined && !isNaN(Number(input.topSimilar))) {
    options.topSimilar = Math.max(1, Math.min(20, Number(input.topSimilar)));
  }
  if (input.minSimilarity !== undefined && !isNaN(Number(input.minSimilarity))) {
    options.minSimilarity = Math.max(0, Math.min(100, Number(input.minSimilarity)));
  }
  let target;
  if (tileId && typeof tileId === "string") {
    target = tileId;
  } else if (tile && typeof tile === "object") {
    if (!tile.body && !tile.recipe) {
      return {
        status: 400,
        data: { error: "invalid_tile", message: "tile 对象至少需包含 body 或 recipe 字段" }
      };
    }
    target = tile;
  } else {
    return {
      ...
      data: {
        error: "missing_input",
        message: "需提供 tileId 或 tile 对象",
        required: ["tileId (string)", "tile ({body, recipe, ...})"]
      }
    };
  }
  const result = generateExperimentReview(db, target, options);
  if (result.error) {
    if (result.error === "tile_not_found") {
      return { status: 404, data: result };
    }
