...
export function validateRows(parsed, existingIds) {
  ...
  for (const row of rows) {
    ...
    if (row.materialBatchRefs !== undefined && row.materialBatchRefs !== null && row.materialBatchRefs !== "") {
      if (!Array.isArray(row.materialBatchRefs)) {
        rowErrors.push("materialBatchRefs 需为数组");
      } else {
        for (let mi = 0; mi < row.materialBatchRefs.length; mi++) {
          const ref = row.materialBatchRefs[mi];
          if (!ref || typeof ref !== "object") {
            rowErrors.push(`materialBatchRefs 第${mi + 1}项必须为对象`);
          } else {
            if (!ref.ingredientName) rowErrors.push(`materialBatchRefs 第${mi + 1}项缺少 ingredientName`);
            if (!ref.batchNo) rowErrors.push(`materialBatchRefs 第${mi + 1}项缺少 batchNo`);
          }
        }
      }
    }

    if (row.batchWeight !== undefined && row.batchWeight !== null && row.batchWeight !== "") {
      const bw = Number(row.batchWeight);
      if (isNaN(bw) || bw <= 0) {
        rowErrors.push("batchWeight 需为正数");
      }
    }
...
function normalizeRow(row) {
  ...
  const materialBatchRefs = Array.isArray(row.materialBatchRefs)
    ? row.materialBatchRefs.map(r => ({
        ingredientName: String(r.ingredientName || "").trim(),
        batchNo: String(r.batchNo || "").trim()
      }))
    : null;

  const batchWeight = row.batchWeight !== undefined && row.batchWeight !== null && row.batchWeight !== ""
    ? Number(row.batchWeight)
    : null;
