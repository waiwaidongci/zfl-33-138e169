...
import { summarizeTiles, diffTilePerformance, buildTilesByVersionIndex } from "./reports.js";
...
function findSimilarSuccessTiles(tile, allTiles, options = {}) {
  ...
  const rawTiles = enriched.map(e => e.tile);
  const groupSummary = rawTiles.length > 0 ? summarizeTiles(rawTiles) : null;
  const highFreqDefects = rawTiles.length > 0 ? getHighFrequencyDefects(rawTiles, 5) : [];
  ...
  return {
    ...
    groupSummary,
    highFrequencyDefects: highFreqDefects
  };
}
...
function analyzeDefectPatterns(tile, allTiles) {
  ...
  if (others.length > 0) {
    const historyDefectSummary = getDefectSummaryForTiles(others);
    const historyHighFreqDefects = getHighFrequencyDefects(others, 10);
    analysis.historyDefectSummary = historyDefectSummary;
    analysis.historyTopDefects = historyHighFreqDefects;
  }
...
export function generateExperimentReview(db, tileIdOrTile, options = {}) {
  ...
  let recipeVersionAnalysis = null;
  try {
    const tilesByVersion = buildTilesByVersionIndex(db);
    const versionKey = targetTile.recipeVersionId || targetTile.recipe || "__unassigned__";
    const sameVersionTiles = tilesByVersion[versionKey] || [];
    const otherVersionTiles = allTiles.filter(t =>
      t.id !== targetTile.id && (t.recipeVersionId || t.recipe || "__unassigned__") !== versionKey
    );
    if (sameVersionTiles.length > 0 || otherVersionTiles.length > 0) {
      const sameVersionSummary = summarizeTiles(sameVersionTiles);
      const otherVersionSummary = summarizeTiles(otherVersionTiles);
      const performanceDiff = diffTilePerformance(sameVersionSummary, otherVersionSummary);
      recipeVersionAnalysis = {
        versionKey,
        sameVersionCount: sameVersionTiles.length,
        sameVersionSummary,
        otherVersionSummary,
        performanceDiff,
        analysisNote: sameVersionTiles.length < 3
          ? `同配方版本样本较少（${sameVersionTiles.length} 片），版本维度结论仅供参考`
          : null
      };
    }
  } catch (_) {
    recipeVersionAnalysis = { error: "recipe_version_analysis_failed" };
  }

  const targetAsArray = [targetTile];
  const targetSummary = summarizeTiles(targetAsArray);
  const similarGroupSummary = similarResult.groupSummary || summarizeTiles([]);
  const performanceVsSimilar = diffTilePerformance(targetSummary, similarGroupSummary);
  ...
  return {
    ...
    similarSuccessfulTiles: {
      ...
      groupSummary: similarResult.groupSummary,
      highFrequencyDefects: similarResult.highFrequencyDefects,
      reportsIntegration: {
        summarizeTilesApplied: !!similarResult.groupSummary,
        getHighFrequencyDefectsApplied: Array.isArray(similarResult.highFrequencyDefects)
      }
    },
    riskFactors: {
    ...
    allEvidence: [
      ...
      {
        rule: "REPORTS_SUMMARIZE_TILES",
        source: "reports.summarizeTiles",
        description: "相似成功样片组统计摘要（平均分/最高分/缺陷率等）",
        data: similarResult.groupSummary
      },
      {
        rule: "REPORTS_HIGH_FREQ_DEFECTS",
        source: "defect-statistics.getHighFrequencyDefects (via reports)",
        description: "相似成功样片组高频缺陷排名",
        data: similarResult.highFrequencyDefects
      },
      {
        rule: "REPORTS_DIFF_TILE_PERFORMANCE",
        source: "reports.diffTilePerformance",
        description: "目标试片与相似成功样片组的性能差异对比",
        data: performanceVsSimilar
      },
      ...(recipeVersionAnalysis && !recipeVersionAnalysis.error ? [
        {
          rule: "REPORTS_BUILD_TILES_BY_VERSION_INDEX",
          source: "reports.buildTilesByVersionIndex + summarizeTiles",
          description: "配方版本维度的性能对比分析",
          data: recipeVersionAnalysis
        }
      ] : [])
    ]
  };
