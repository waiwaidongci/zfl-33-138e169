...
export function reserveStock(db, tile, deductions) {
  ...
  for (const d of deductions) {
    ...
    if (available < d.requiredQuantity) {
      for (const rs of reservedStocks) {
        rs.stock.reservedQuantity = Number((rs.stock.reservedQuantity - rs.quantity).toFixed(2));
      }
      for (const txnId of transactionIds) {
        const idx = coll.inventoryTransactions.findIndex(t => t.id === txnId);
        if (idx >= 0) coll.inventoryTransactions.splice(idx, 1);
      }
      throw new Error(`原料 '${d.ingredientName}' 可用库存不足，无法预留: 需要 ${d.requiredQuantity}, 可用 ${available} (总量 ${stock.quantity}, 已预留 ${stock.reservedQuantity || 0})`);
    }

    stock.reservedQuantity = Number(((stock.reservedQuantity || 0) + d.requiredQuantity).toFixed(2));
    reservedStocks.push({ stock, quantity: d.requiredQuantity });
  ...
  recordEvent(db, {
    type: EVENT_TYPES.INVENTORY_RESERVED,
    entityId: tile.id,
...
export function confirmStockReservation(db, tile) {
  ...
  for (const reserveTxn of activeReserveTxns) {
    ...
    if (stock.quantity < reserveTxn.quantity) {
      for (const cs of confirmedStocks) {
        cs.stock.quantity = Number((cs.stock.quantity + cs.quantity).toFixed(2));
        cs.stock.reservedQuantity = Number((cs.stock.reservedQuantity + cs.quantity).toFixed(2));
        cs.reserveTxn.status = "active";
      }
      for (const txnId of confirmedTxnIds) {
        const idx = coll.inventoryTransactions.findIndex(t => t.id === txnId);
        if (idx >= 0) coll.inventoryTransactions.splice(idx, 1);
        if (Array.isArray(tile.reservationIds)) {
          const rIdx = tile.reservationIds.indexOf(txnId);
          if (rIdx >= 0) tile.reservationIds.splice(rIdx, 1);
        }
      }
      for (const cs of confirmedStocks) {
        const ref = (tile.materialBatchRefs || []).find(
          r => r.ingredientName === cs.reserveTxn.ingredientName && r.batchNo === cs.reserveTxn.batchNo
        );
        if (ref) delete ref.deducted;
      }
    ...
    confirmedTxnIds.push(confirmTxnId);
    confirmedStocks.push({ stock, quantity: reserveTxn.quantity, reserveTxn });
  ...
  ensureEventCollection(db);
  recordEvent(db, {
    type: EVENT_TYPES.INVENTORY_CONFIRMED,
    entityId: tile.id,
    entityType: ENTITY_TYPES.TILE,
    payload: {
      confirmedIds,
    ...
    operator: "system",
    note: `确认消耗 ${confirmedIds.length} 种原料`
  });
...
export function releaseStockReservation(db, tile) {
  ...
  ensureEventCollection(db);
  recordEvent(db, {
    type: EVENT_TYPES.INVENTORY_RELEASED,
    entityId: tile.id,
    entityType: ENTITY_TYPES.TILE,
    payload: {
      releasedIds,
    ...
    operator: "system",
    note: `释放 ${releasedIds.length} 种原料预留`
  });
