...
import { isFieldAllowed, validateFieldsForStatus } from "./tile-permission-rules.js";
...
export async function handleUpdateTileDefectTags(tileId, input, db) {
  ...
  const fields = ["defectTags"];
  if (input.defects !== undefined) fields.push("defects");
  const fieldValidation = validateFieldsForStatus(tile.status, fields);
  if (!fieldValidation.valid) {
    return {
      status: 400,
      data: {
        error: "fields_not_allowed",
        message: fieldValidation.errors.map(e => e.message).join("; "),
        currentStatus: tile.status,
        allowedFields: fieldValidation.allowedFields
      }
    };
  }
  ...
  recordEvent(db, {
    type: EVENT_TYPES.DEFECT_TAGS_CHANGED,
    entityId: tile.id,
    ...
    operator: "user",
    note: `更新缺陷标签: ${tile.defectTags.map(t => t.name).join("、") || "清空"}`
  });
...
export async function handleAddDefectTag(tileId, input, db) {
  ...
  if (!isFieldAllowed(tile.status, "defectTags")) {
    return {
      status: 400,
      data: {
        error: "fields_not_allowed",
        message: `当前状态为 '${tile.status}'，不允许修改缺陷标签`
      }
    };
  }
  ...
  recordEvent(db, {
    ...
    payload: {
      action: "add",
      addedTag: { name: normalizedName, severity: normalizeSeverity(input.severity) },
      defectTags: tile.defectTags
    },
    operator: "user",
    note: `添加缺陷标签: ${normalizedName}(${normalizeSeverity(input.severity)})`
  });
...
export async function handleRemoveDefectTag(tileId, input, db) {
  ...
  recordEvent(db, {
    ...
    payload: {
      action: "remove",
      removedTag: nameToRemove,
      defectTags: tile.defectTags
    },
    operator: "user",
    note: `移除缺陷标签: ${nameToRemove}`
  });
