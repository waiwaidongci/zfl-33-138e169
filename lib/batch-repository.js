...
export function getBatchIds(db) {
  ...
  return new Set(db.batches.map(b => b.id));
}

export function generateBatchId(db) {
  ensureBatchCollection(db);
  let counter = db.batches.length + 1;
  let id;
...
export function generateBatchSummary(db, batchId) {
  ...
  const missingTileIds = batch.tileIds.filter(tid => !db.tiles.find(t => t.id === tid));
