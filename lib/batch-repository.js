...
export function addTilesToBatchInBulk(db, batchId, tileIds) {
  const results = {
    added: [],
    duplicated: [],
    notFound: [],
  ...
  const batch = getBatch(db, batchId);
  if (!batch) {
    return { ...results, error: "batch_not_found" };
  }

  const coll = getCollections(db);
  const existingTileIds = new Set(coll.tiles.map(t => t.id));

  for (const tileId of tileIds) {
    if (!existingTileIds.has(tileId)) {
      results.notFound.push(tileId);
      continue;
    }

    const tile = coll.tiles.find(t => t.id === tileId);
    if (tile && !isFieldAllowed(tile.status, "batchId")) {
      results.forbidden.push({ id: tileId, status: tile.status });
      continue;
    }

    const result = addTileToBatch(db, batchId, tileId);
    if (result.duplicate) {
      results.duplicated.push(tileId);
    } else {
      if (tile) tile.batchId = batchId;
      results.added.push(tileId);
    }
  }
