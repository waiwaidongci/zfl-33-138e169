...
export async function handleGetBatch(id, db) {
  ...
  const tiles = batch.tileIds
    .map(tid => db.tiles.find(t => t.id === tid))
    .filter(Boolean);
...
export async function handleAddBatchTiles(id, input, db) {
  ...
  const existingIds = new Set(db.tiles.map(t => t.id));
  ...
  await saveDb(db);
  return { status: 200, data: { added, duplicated, batch: getBatch(db, id) } };
}
...
export async function handleRemoveBatchTiles(id, input, db) {
  ...
  await saveDb(db);
  return { status: 200, data: { removed, notInBatch, batch: getBatch(db, id) } };
}
