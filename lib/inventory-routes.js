...
export async function handleGetTileTransactions(tileId, db) {
  ensureInventoryCollection(db);
  const transactions = getTransactionsByTileId(db, tileId);
  return { status: 200, data: { tileId, transactions } };
}

export async function handleGetStockTransactions(stockId, db) {
  ensureInventoryCollection(db);
  const stock = getStock(db, stockId);
  if (!stock) return { status: 404, data: { error: "stock_not_found", message: `库存记录 '${stockId}' 不存在` } };
  const transactions = getTransactionsByStockId(db, stockId);
  return { status: 200, data: { stockId, materialName: stock.name, batchNo: stock.batchNo, transactions } };
}
